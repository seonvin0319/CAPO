#!/usr/bin/env bash
# v8 hold success config × 9 locomotion cells with IQL (seed 0, sequential).
# Config: configs/v8_hold.yaml  (λ_D=0.2, λ_T=1.0, period=50k, margin=0.0, …)
# Distinct from choi's td3_bc v8_hold matrix (host/choi.csv).
set -euo pipefail
...
echo "[CAPO v8_hold IQL] 9 jobs → $OUT_DIR (config=$CONFIG seed=$SEED device=$DEVICE)"
...
echo "[CAPO v8_hold IQL] finished. status=$STATUS_FILE"
