#!/usr/bin/env bash
# CAPO vs IQL baseline on locomotion (matched to defaults schedule).
# 2 variants × 3 envs × 3 datasets = 18 runs (seed 0, sequential).
#   capo      → configs/defaults.yaml       (use_capo: true,  --algorithm iql)
#   baseline  → configs/baseline_iql.yaml   (use_capo: false, --algorithm iql)
set -euo pipefail
...
VARIANTS=(
  ...
  "baseline|configs/baseline_iql.yaml|baseline"
)

STATUS_FILE="$OUT_DIR/queue_status_iql.tsv"
mkdir -p "$OUT_DIR"
...
latest_run_dir() {
  ...
  # Prefer results/<algo>/<env>/s<seed>/; fall back to legacy results/<env>/s<seed>/
  ...
  [[ -n "$hit" ]] || return 1
  # Resolve symlink → canonical path when present
  if [[ -L "$hit" ]]; then
    hit="$(readlink -f "$hit")"
  fi
  echo "$hit"
...
echo "[CAPO IQL matrix] 18 jobs (capo + baseline iql × 9 envs) → $OUT_DIR (seed=$SEED device=$DEVICE)"
...
echo "[CAPO IQL matrix] finished. status=$STATUS_FILE"
