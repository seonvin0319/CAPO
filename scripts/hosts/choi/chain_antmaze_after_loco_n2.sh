...
# then launch AntMaze v2 replace_new mgrid (72 cells) with the same settings.
set -euo pipefail
ROOT=/home/choi/CAPO
...
ANT_ROOT="$ROOT/results_jax_sweeps/capo_stability_seed0_fast_n2_antmaze"
ANT_MANIFEST="$ROOT/manifests/capo_stability_seed0_fast_n2_antmaze_replace_new_mgrid.jsonl"
LOG="$ANT_ROOT/chain_waiter.log"
mkdir -p "$ANT_ROOT"
cd "$ROOT"
...
echo "[$(date -Is)] chain waiter started" | tee -a "$LOG"
...
for ent in Path("/proc").iterdir():
    ...
    try:
        ...
        continue
    if "run_capo_jax.py" in cmd and "capo_stability_seed0_fast_n2/" in cmd and "_antmaze" not in cmd:
        live += 1
print(live)
...
while true; do
  if loco_complete >>"$LOG" 2>&1; then
    if loco_trainers_live >>"$LOG" 2>&1; then
      echo "[$(date -Is)] locomotion complete; launching antmaze" | tee -a "$LOG"
      break
    fi
...
# stop loco master if still around (should be exiting)
...
nohup "$PY" scripts/run_capo_stability_sweep.py \
  --manifest "$ANT_MANIFEST" \
  --gpus 0 --jobs_per_gpu 2 \
  --override n_critics=2 \
  --override sweep_name=capo_stability_seed0_fast_n2_antmaze \
  --override save_best=false \
  --override save_refresh_actors=false \
  --python "$PY" \
  --mujoco_py_source /tmp/no_such_mujoco_py_for_choi \
  --resume \
  >> "$ANT_ROOT/queue.log" 2>&1 &
echo $! > "$ANT_ROOT/queue.pid"
echo "[$(date -Is)] antmaze master=$(cat "$ANT_ROOT/queue.pid")" | tee -a "$LOG"
