...
#   2) stop the live 18-cell matrix so it does NOT start baseline
#   3) run v8_hold × 9
#   4) run baseline td3bc × 9
set -euo pipefail
...
stop_live_matrix() {
  ...
  if [[ -n "${master}" ]] && kill -0 "$master" 2>/dev/null; then
    echo "[$(date '+%F %T')] stopping matrix master pid=$master (prevent baseline phase)"
    kill "$master" 2>/dev/null || true
    sleep 2
    kill -9 "$master" 2>/dev/null || true
  fi
  # If baseline already spawned, kill trainers belonging to this repo.
...
{
  echo "[$(date '+%F %T')] choi pipeline start: capo9 → v8_hold9 → baseline9"
  ...
  echo "[$(date '+%F %T')] phase2: launch v8_hold × 9"
  nohup bash "$ROOT/scripts/run_matrix_v8_hold.sh" > "$ROOT/results/queue_v8_hold.log" 2>&1 &
  echo $! > "$ROOT/results/queue_v8_hold.pid"
  wait_pid_file "$ROOT/results/queue_v8_hold.pid" "v8_hold"

  echo "[$(date '+%F %T')] phase3: launch baseline td3bc × 9"
