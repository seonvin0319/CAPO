#!/usr/bin/env bash
# v8 hold success config × 9 locomotion cells with CQL (seed 0, sequential).
# Config: configs/v8_hold.yaml  (λ_D=0.2, λ_T=1.0, period=50k, margin=0.0, …)
# svcho owns CQL; distinct from choi/ext_csh td3_bc and offrl IQL v8_hold.
set -euo pipefail
...
STATUS_FILE="$OUT_DIR/queue_status_v8_hold_cql.tsv"
mkdir -p "$OUT_DIR"
...
echo "[CAPO v8_hold CQL] 9 jobs → $OUT_DIR (config=$CONFIG seed=$SEED device=$DEVICE python=$PYTHON)"
...
for envb in "${ENVS[@]}"; do
  for ds in "${DATASETS[@]}"; do
    ...
    echo "[run] $VARIANT $ALGO $envb $ds → $env_id"
    set +e
...
echo "[CAPO v8_hold CQL] finished. status=$STATUS_FILE"
